...
/* ══════════════════════════════════════════════════════════
   REPARACIÓN DE colegioId — corrige usuarios con colegio incorrecto
   POST /api/superadmin/repair-colegios
   Busca cada colegio, encuentra usuarios con colegioId incorrecto
   basándose en sus salones asignados, y los corrige.
══════════════════════════════════════════════════════════ */
router.post('/repair-colegios', async (req, res) => {
  try {
    const colegios = await Colegio.find().lean();
    const report = [];
    let totalFixed = 0;
    ...
    for (const col of colegios) {
      ...
      if (!salonNames.length) continue;

      const colReport = { colegio: cnombre, id: cid, fixedEsts: 0, fixedProfs: 0 };

      // Reparar estudiantes con salón correcto pero colegioId incorrecto
      const estsWrong = await Usuario.find({
        role: 'est',
        salon: { $in: salonNames },
        colegioId: { $ne: cid },
      }).lean();
      if (estsWrong.length) {
        const r = await Usuario.updateMany(
          { _id: { $in: estsWrong.map(e => e._id) } },
          { $set: { colegioId: cid, colegioNombre: cnombre } }
        );
        colReport.fixedEsts = r.modifiedCount;
        totalFixed += r.modifiedCount;
      }

      // Reparar profes con salones correctos pero colegioId incorrecto
      const profsWrong = await Usuario.find({
        role: 'profe',
        salones: { $elemMatch: { $in: salonNames } },
        colegioId: { $ne: cid },
      }).lean();
      if (profsWrong.length) {
        const r = await Usuario.updateMany(
          { _id: { $in: profsWrong.map(p => p._id) } },
          { $set: { colegioId: cid, colegioNombre: cnombre } }
        );
        colReport.fixedProfs = r.modifiedCount;
        totalFixed += r.modifiedCount;
      }

      if (colReport.fixedEsts || colReport.fixedProfs) {
        report.push(colReport);
        Auditoria.create({
          ts: new Date().toISOString(), uid: 'superadmin', who: 'superadmin',
          role: 'superadmin',
          accion: `Reparación colegioId: ${colReport.fixedEsts} ests + ${colReport.fixedProfs} profs en "${cnombre}"`,
          colegioId: cid
        }).catch(() => {});
      }
    }

    res.json({ ok: true, totalFixed, report });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
